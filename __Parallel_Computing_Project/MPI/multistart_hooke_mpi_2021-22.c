...
int main(int argc, char *argv[])
{
	...
	int nvars;
	int trial, ntrials;
	int i, j, jj; // η μεταβλητη j θα χρησιμοποιηθει στη συνεχεια μεσα σε for-loop
	double t0, t1, fx;
	...
	MPI_Status mpi_status; // Μεταβληρή για να γνωρίζουμε την κατάσταση των μηνυμάτων
	...
	for (trial = 0; trial < ntrials / p; trial++) /* με αυτή την εντολή κάνουμε χωρισμό του αριθμού των trials σε κάθε διεργασία Σχόλιο: αν δεν πραγματοποιούσαμε αυτόν τον χωρισμό, τότε κάθε διεργασία Θα εκτελούσε τόσα trials όσα και τιμή της μεταβλητής ntrials*/
	{
		...
		if (fx < best_fx)
		{
			...
			for (i = 0; i < nvars; i++)
				best_pt[i] = endpt[i];
		} // καθε διεργασια process κανει υπολογισμο των καλυτερων τιμων της ωστε να γινει ελαττωση του πληθους των μηνυματων που θα αποστελλονται και να αποφευχθει να "κολλησει" το προγραμμα (δηλαδη να "παγωσει" (να freezeαρει))
	}
	...
	else
	{
		...
		{
			message_source = j;
			MPI_Recv(&fx, 1, MPI_DOUBLE, message_source, tag, MPI_COMM_WORLD, &mpi_status); /* γινεται αποδοχη των δεδομενων της καθε διεργασιας με σειριακο τροπο  */
		...
		printf("Total number of function evaluations = %ld\n", funevals * p); //  οπως και στο OpenMP , ετσι και σε αυτη εδω τη περιπτωση κανουμε πολλαπλασιασμο του αριθμου των πράξεων με τον αριθμό των διεργασιων (processes) προκειμενου να υπολογισουμε τον πραγματικό αριθμό των πράξεων που γινονται
