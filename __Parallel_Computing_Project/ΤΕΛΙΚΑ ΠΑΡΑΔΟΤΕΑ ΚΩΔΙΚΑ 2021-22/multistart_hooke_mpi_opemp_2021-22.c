...
#include <time.h>
#include <sys/time.h>
#include <omp.h> // // ενσωματωση της βιβλιοθηκης (αρχειου κεφαλιδας) του OpenMP
#include <mpi.h>
...
int main(int argc, char *argv[])
{
	...
	int trial, ntrials;
	double fx;
	int i, jj, j; // η μεταβλητη j θα χρησιμοποιηθει στη συνεχεια ως μετρητης σε for-loop
	double t0, t1;
	...
	int destination = 0; // Μεταβληρή για να ορίζουμε τον προορισμό των μηνυμάτων
	MPI_Status status; // Μεταβληρή για να γνωρίζουμε την κατάσταση των μηνυμάτων

	for (i = 0; i < MAXVARS; i++)
		best_pt[i] = 0.0;

	ntrials = 4 * 1024; /* number of trials */
	nvars = 32;			  /* number of variables (problem dimension) */
	t0 = get_wtime();
	int provided;
	int num_of_threads = 6;
	MPI_Init(&argc, &argv); /* με αυτη την εντολη γινεται εκκινηση διαμοιρασμοιυ process (διεργασιας) */
	...
	srand48(my_id + 1); /* με αυτή εδώ την εντολή σιγουρεύουμε ότι σε κάθε διεργασία οι τιμές θα είναι πράγματι τυχαίες */
#pragma omp parallel num_threads(num_of_threads) firstprivate(fx, jj, i) /* εδω αρχιζει ο παραλληλισμος και η δημιουργια νηματων με την παραμετρο num_threads(int). Επιπλεον, γινεται χρηση της μεθοδου/συναρτησης firstprivate για να κανουμε ορισμο των μεταβλητων που δεν επιθυμουμε να γινεται διαμοιραση μεταξυ των νηματων (threads)*/
	{
#pragma omp for // εδω κανουμε παραλληλοποιηση του for-loop  
		for (trial = 0; trial < ntrials / p; trial++) /* πραγματοποιουμε χωρισμο του αριθμου/πληθους των trials σε καθε διεργασια (process). Σημειωση: αν δε πραγματοποιειτο ο χωρισμος αυτος, τότε κάθε διεργασία (process) Θα εκτελούσε τόσα trials όσα και τιμή της μεταβλητής ntrials  */
		{
			...
			printf("f(x) = %15.7le\n", fx);
#endif
#pragma omp critical /*mutex area (περιοχη δυαδικου σημαφορου) για τις μεταβλητες που μας δινουν το τελικο αποτελεσμα , για παραδειγμα αν εχουμε 2 νεα best ταυτοχρονως, τοτε θα εμφανιστει θεμα/προβλημσ στην εκτελεση του προγραμματος*/
			{
				if (fx < best_fx)
				{
	...
	if (my_id != 0) // Όλες οι διεργασιες οι οποιες δεν έχουν id ισο με 0 θα κανουν αποστολη των αποτελεσματων τους στη διεργασια-αφεντη (master process) η οποια εχει id = 0
	{
		MPI_Send(&best_fx, 1, MPI_DOUBLE, destination, tag, MPI_COMM_WORLD); //  πραγματοποιειται αποστολη των δεδομενων των διεργασιων-εργατων (workers-processes) στη διεργασια-αφεντη (master process)
	...
	{
		for (j = 1; j < p; j++) //  αυτό το for-loop αναφερεται για κάθε διεργασια-εργατη (worker-process)
		{
			message_source = j;
			MPI_Recv(&fx, 1, MPI_DOUBLE, message_source, tag, MPI_COMM_WORLD, &status); // γινεται αποδοχη των δεδομενων της καθε διεργασιας με σειριακο τροπο
			...
			if (fx < best_fx) // κατ'αρχην κανουμε συγκριση των δεδομενων της διεργασιας-αφεντη (master process) με τη διεργασια που εχει id=1 και στη συνεχεια σειριακα με τις υπολοιπες διεργασιες
			{
				best_trial = trial;
				best_jj = jj;
				best_fx = fx;
		...
		printf("Total number of function evaluations = %ld\n", funevals * p); // κανουμε πολλαπλασιασμο του αριθμου των πράξεων με τον αριθμό των διεργασιων (processes) προκειμενου να υπολογισουμε τον πραγματικό αριθμό των πράξεων που γινονται  
