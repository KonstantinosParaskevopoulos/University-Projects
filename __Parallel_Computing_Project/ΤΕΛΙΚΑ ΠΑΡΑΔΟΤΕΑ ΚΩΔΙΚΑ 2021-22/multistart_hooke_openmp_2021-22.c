...
int main(int argc, char *argv[])
{
    ...
    #pragma omp parallel num_threads(num_of_threads) firstprivate(fx, jj, i) /*Εκκίνηση παραλληλισμού και δημιουργία νημάτων με την παράμετρο num_threads(int), επίσης χρησιμοποιούμε την μέθοδο firstprivate για να ορίσουμε τις μεταβλητές που δεν θέλουμε να διαμοιράζονται μεταξύ των threads*/
    {
         #pragma omp for /*Παραλληλοποίηση βρόχου*/
            for (trial = 0; trial < ntrials; trial++) {
                ...
                printf("f(x) = %15.7le\n", fx);
        #endif
            #pragma omp critical /*mutex area (περιοχη δυαδικου σημαφορου) για τις μεταβλητές που μας επιστρέφουν το τελικό αποτέλεσμα, για παραδειγμα αν έχουμε 2 νέα best ταυτόχρονα θα υπάρξει πρόβλημα*/
            {  		
                    if (fx < best_fx) {
                        best_trial = trial;
                        best_jj = jj;
	...
	printf("Total number of function evaluations = %ld\n", funevals * num_of_threads); /*Πολλαπλασιασμό με αριθμό threads για να βρούμε το πραγματικό αριθμό των πράξεων (αφού το αρχικό ποσό μοιράζεται σε n threads*/
