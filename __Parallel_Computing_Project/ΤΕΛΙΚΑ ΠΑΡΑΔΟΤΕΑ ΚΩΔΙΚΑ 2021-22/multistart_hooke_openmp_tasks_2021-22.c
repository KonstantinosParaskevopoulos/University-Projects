...
int main(int argc, char *argv[])
{
	...
	int num_of_threads = 8; //δηλωση νηματων
	
	
    #pragma omp parallel num_threads(num_of_threads)  // εναρξη του παραλληλισμου και δημιουργια νηματων μεσω της ακεραιας (int) παραμετρου number_of_threads. Κανουμε χρηση της συναρτησης/μεθοδου firstprivate ωστε να ορισουμε τις μεταβλητες που δεν επιθυμουμε να διαμοιραζονται μεταξυ των νηματων
    {
         #pragma omp single nowait // σε αυτο το σημειο λεμε οτι το ακολουθο block απο κωδικα δυναται να γινει απο οποιοδηποτε νημα διχως να αναμενει να τελειωσουν και τα αλλα (σχολιο: αυτο ειναι εξαιρετικα βοηθητικο σε loops  που δεν εχουν ορισμενο καποιον συγκεκριμενο αριθμο επαναληψεων (repetitions) )
            for (trial = 0; trial < ntrials; trial++) {
				#pragma omp task firstprivate(fx, jj, i) // εναρξη tasks ωστε να πραγματοποιηθει υπολογισμος της συναρτησης
				{
					...
					printf("f(x) = %15.7le\n", fx);
			#endif
				#pragma omp critical /* mutex area (περιοχη δυαδικου σημαφορου) για τις μεταβλητες που μας δινουν το τελικο αποτελεσμα , για παραδειγμα αν εχουμε 2 νεα best ταυτοχρονως, τοτε θα εμφανιστει θεμα/προβλημa */
				{  
						if (fx < best_fx) {
							best_trial = trial;
							best_jj = jj;
	...
	printf("Total number of function evaluations = %ld\n", funevals * num_of_threads); // πραγματοποιειται πολλαπλασιασμος με τον αριθμο των νηματων (threads) προκειμενου να υπολογιστει το πραγματικο πληθος των πραξεων (αφου το αρχικο ποσο μοιραζεται σε n (το πληθος) νηματα
